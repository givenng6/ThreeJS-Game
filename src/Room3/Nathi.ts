...
function Nathi() {
  ...
  var interval = setInterval(function () {
    if(!pause){
    timer--;
    if (parseInt(document.getElementById("timerText").innerHTML) !== 0 && !win) {
  ...
  function ShowSecondGame(bool:boolean) {
    for (var i = 0; i < 20; i++) {
      scene.getObjectByName(i.toString() + "box").visible = bool;
      scene.getObjectByName(i.toString() + "letter").visible = bool;
      // console.log(scene.children)
      // scene.children[2] = new THREE.Mesh(new THREE.BoxGeometry(), new THREE.MeshBasicMaterial({color: new THREE.Color(0,0,0)}))
    }
    ...
    if(bool){
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (intersects.length > 0) {
          var tmp: any = intersects[0];
          
          // tmp.object.material.color.set("0xbd9fb9")
          
          if(SELECTEDINDEX.indexOf(i)!==-1){
            tmp.object.material.color.r = 0;
            tmp.object.material.color.g = 0;
            tmp.object.material.color.b = 0;
            if(letterBoxIndex.indexOf(i.toString()) !== -1){
              CORRECTCOUNT--;
            }
            // SELECTED = SELECTED.filter((item,index) =>)
          }else{
            PRESSEDCOUNT++;
            tmp.object.material.color.r = 1;
            tmp.object.material.color.g = 0;
            tmp.object.material.color.b = 0;
            SELECTED.push(tmp.object.material.color);
          ...
          // if (tmp.object.material.color.r)
